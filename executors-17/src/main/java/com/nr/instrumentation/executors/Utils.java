...
import com.nr.instrumentation.util.concurrent.ForkJoinTask_instrumentation;
...
public class Utils {
	...
	public static String EXTPUSHMETHODDESC = "(Ljava/util/concurrent/ForkJoinTask;)V";
	...
	static {
		...
		ignoredPackages.add(NEWRELICAGENT);
		...
		Class<?> forkJoinPoolClass = ForkJoinPool.class;
		Method matching = null;
		
		Method[] methods = forkJoinPoolClass.getDeclaredMethods();
		for(int i=0;i<methods.length && matching == null;i++) {
			Method method = methods[i];
			if(method.getName().equals("externalPush")) {
				matching = method;
			}
		}
		
//		if(matching != null) {
//			EXTPUSHMETHODDESC = Type.getMethodDescriptor(matching);
//			NewRelic.getAgent().getLogger().log(Level.FINE, "Set EXTPUSHMETHODDESC to {0}", EXTPUSHMETHODDESC);
//		}
	}
	
	public static boolean ignorePackage(String classname) {
