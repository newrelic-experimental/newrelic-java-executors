...
public abstract class ForkJoinPool_instrumentation {
	...
	@Trace
	public <T> ForkJoinTask_instrumentation<T> externalSubmit(ForkJoinTask_instrumentation<T> task) {
	...
	static final class WorkQueue_instrumentation {
		...
		void push(ForkJoinTask_instrumentation<?> task,ForkJoinPool_instrumentation pool, boolean signalIfEmpty) {
			if (!Utils.isCompletion(task)) {
				if (task.token == null && !task.alreadyWrapped) {
