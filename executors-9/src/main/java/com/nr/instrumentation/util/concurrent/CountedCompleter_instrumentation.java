...
public abstract class CountedCompleter_instrumentation<T> extends ForkJoinTask_instrumentation<T> {
	...
	protected boolean exec() {
		...
		Utils.logForkJoinTask("CountedCompleter call to {0}.exec(), pending={1}, completer={2}, thread={3}", getClass().getSimpleName(),pending,completer,Thread.currentThread().getName());
